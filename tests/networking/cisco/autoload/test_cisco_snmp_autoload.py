...
class TestsCiscoGenericSNMPAutoload(TestCase):
    def setUp(self):
        ...
        self.cisco_snmp_autoload = CiscoGenericSNMPAutoload(
            self._snmp_handler,
            self._shell_name,
            self._shell_type,
            self._resource_name,
    ...
    def test_load_snmp_tables(self, snmp_ent_tbl_mock, snmp_if_tbl_mock):
        snmp_ent_tbl_mock.get_entity_table.return_value = QualiMibTable(
            "EntPhysicalTable", {1: {"entPhysicalClass": "chassis"}}
        )
        self.cisco_snmp_autoload._load_snmp_tables()
        snmp_if_tbl_mock.return_value.called_once_with(self._snmp_handler, self._logger)
        snmp_ent_tbl_mock.return_value.called_once_with(
            self._snmp_handler, self._logger, snmp_if_tbl_mock
        )
    ...
    def test_add_element(self):
        ...
        self.assertTrue(
            self.cisco_snmp_autoload.resource.resources["CH"][relative_path][-1]
            == chassis
        )
        ...
        self.assertTrue(
            self.cisco_snmp_autoload.resource.resources["CH"][relative_path][
                -1
            ].resources["P"][relative_path][-1]
        )

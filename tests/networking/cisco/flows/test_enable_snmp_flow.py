...
class TestCiscoEnableSNMPFlow(TestCase):
    ...
    def _get_handler(self, create_group=True):
        ...
        cli = MagicMock()
        logger = MagicMock()
        return CiscoEnableSnmpFlow(
            cli_handler=cli, logger=logger, create_group=create_group
        )
    ...
    def test_enable_snmp_v3(self, enable_actions_mock):
        ...
        enable_snmp_mock.assert_called_once_with(
            ...
            auth_protocol=CiscoEnableSnmpFlow.SNMP_AUTH_MAP[
                self.snmp_v3_parameters.auth_protocol
            ].lower(),
            priv_protocol=CiscoEnableSnmpFlow.SNMP_PRIV_MAP[
                self.snmp_v3_parameters.private_key_protocol
            ].lower(),
    ...
    def test_validate_snmp_v3_params_validates_user_and_raise(self):
        ...
        try:
            ...
            self.assertIn(
                "Failed to enable SNMP v3: 'SNMP V3 User' attribute cannot be empty",
                e.args,
            )

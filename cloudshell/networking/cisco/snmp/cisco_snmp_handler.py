...
class CiscoSnmpHandler(SnmpHandler):
    def __init__(self, cli, resource_config, logger, api):
        super(CiscoSnmpHandler, self).__init__(resource_config, logger)
        self._cli = cli
        self._api = api

    @property
    def cisco_cli_handler(self):
        return CiscoCliHandler(self._cli, self.resource_config, self._logger, self._api)

    def _create_enable_flow(self):
        return CiscoEnableSnmpFlow(self.cisco_cli_handler, self._logger)

    def _create_disable_flow(self):
        return CiscoDisableSnmpFlow(self.cisco_cli_handler, self._logger)

...
class CiscoGenericSNMPAutoload(object):
    ...
    def _load_snmp_tables(self):
        ...
        self.entity_table = self.autoload.entPhysicalTable
        self._logger.info('Entity table loaded')

        self.if_table = self.autoload.ifTable
        self._logger.info('IfTable loaded')

        self.snmp_mib = self.autoload.get_table('SNMPv2-MIB', 'system')
        self.lldp_local_table = self.autoload.get_table('LLDP-MIB', 'lldpLocPortDesc')
        self.lldp_remote_table = self.autoload.get_table('LLDP-MIB', 'lldpRemTable')
        self.cdp_index_table = self.autoload.get_table('CISCO-CDP-MIB', 'cdpInterface')
        self.cdp_table = self.autoload.get_table('CISCO-CDP-MIB', 'cdpCacheTable')
        self.duplex_table = self.autoload.get_table('EtherLike-MIB', 'dot3StatsTable')
        self.ip_v4_table = self.autoload.get_table('IP-MIB', 'ipAddrTable')
        self.ip_v6_table = self.autoload.get_table('IPV6-MIB', 'ipv6AddrEntry')
        self.if_x_table = self.autoload.get_table('IF-MIB', 'ifAlias')
        self.port_channel_ports = self.autoload.get_table('IEEE8023-LAG-MIB', 'dot3adAggPortAttachedAggID')

        self._logger.info('MIB Tables loaded successfully')
        self.port_mapping = self.autoload.get_mapping()
